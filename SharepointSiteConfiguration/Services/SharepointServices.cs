...
namespace SharepointSiteConfiguration.Services
{
    ...
    {
        public async Task UploadPackageFileAsync(AppSettings settings, string accessToken)
        {
            try
            {
                var sppkgFilePath = "";
                ...
                using (var siteContext = new ClientContext(siteUrl))
                {
                    ...
                    Console.WriteLine("App installing...");
                    var targetAppManager = new AppManager(siteContext);
                    var uploadApp = targetAppManager.Add(sppkgFilePath, overwrite: true, AppCatalogScope.Site);
                    targetAppManager.Deploy(uploadApp.Id, true, AppCatalogScope.Site);
                    siteContext.ExecuteQuery();
                    Console.WriteLine("App installed in target site.");
        ...
        public static void IsAppCatalogAvailable(string targetSiteUrl, string accessToken) {
            using (var targetSiteContext = new ClientContext(targetSiteUrl))
            {
                targetSiteContext.ExecutingWebRequest += (sender, e) =>{
                    e.WebRequestExecutor.WebRequest.Headers["Authorization"] = "Bearer " + accessToken;
                };

                try {
                    var appcatalog = targetSiteContext.Site.RootWeb.SiteCollectionAppCatalog;
                    targetSiteContext.Load(appcatalog);
                    targetSiteContext.ExecuteQueryRetry();
                    if ((bool)appcatalog.ServerObjectIsNull)
                    {
                        Console.WriteLine("App Catalog does not exist...");
                    }
                    else
                    {
                        Console.WriteLine("App Catalog is exist...");
                    }
                } 
                catch (Exception e) 
                { 
        ...
        public static void addAppCatalog(string targetSiteUrl, ClientContext siteContext)
        {
            try
            {
                siteContext.Load(siteContext.Site, s => s.RootWeb);
                siteContext.ExecuteQuery();
                siteContext.Site.RootWeb.TenantAppCatalog.SiteCollectionAppCatalogsSites.Add(targetSiteUrl);
                siteContext.ExecuteQuery();
            }
